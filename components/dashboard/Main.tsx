...
const Main = () => {
  ...
  const handleDeleteTraces = (idsToDelete: string[]) => {
    ...
    (window as any).pendo?.track('Trace Deleted', { traceIds: idsToDelete.join(','), deleteCount: idsToDelete.length, isBulk: idsToDelete.length > 1 });
                  ...
                  return (
                    <TraceCard
                      ...
                      onShare={() => {
                        ...
                        (window as any).pendo?.track('Trace Link Copied', { traceId: trace.id, traceTitle: trace.title, isOwner: trace.isOwner });
