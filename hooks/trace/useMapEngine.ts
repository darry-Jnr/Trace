...
export function useMapEngine(
  ...
  const setStartPoint = useCallback((coords: [number, number] | null, color?: "blue" | "green") => {
    ...
    const bg = isGreen ? "#22c55e" : "#0052FF";
    const shadow = isGreen
      ? "0 2px 10px rgba(34,197,94,0.4)"
      : "0 2px 10px rgba(0,82,255,0.4)";
    ...
    const marker = new mapboxgl.Marker({ element: el }).setLngLat(coords).addTo(mapInstanceRef.current);
  ...
  useEffect(() => {
    ...
    // User marker — black dot
    const el = document.createElement("div");
    el.className = "w-5 h-5 rounded-full border-4 border-white shadow-[0_2px_10px_rgba(0,0,0,0.4)] flex items-center justify-center relative";
    el.style.backgroundColor = "#000000";
    el.innerHTML = `<span class="absolute inset-0 rounded-full animate-ping scale-150 pointer-events-none" style="animation-duration: 2s; background-color: #000000;" />`;
  ...
  const injectDOMMarker = (waypoint: WaypointMedia) => {
    ...
    const icons: Record<string, string> = {
      ...
      image: `<svg class="w-4 h-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="12" r="4"></circle></svg>`,
